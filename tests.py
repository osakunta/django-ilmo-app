...
from .models import Event,EventAttendee,Place,Payment,EventCannotAttendException
from datetime import date,datetime,timedelta
from django.utils import timezone

PLACE_MOCK = {'name' : 'Place'}
EVENT_MOCKS = [
{'name' : 'Event', 'form_name' : 'test_form', 'event_date' : date(2015,1,1), 'capacity' : 1, 'close_date' : timezone.now() - timedelta(days=1),'description' : 'empty'},
{'name' : 'Event1', 'form_name' : 'test_form', 'event_date' : date(2015,1,1), 'capacity' : 0, 'close_date' : timezone.now() + timedelta(days=1),'description' : 'empty','backup' : False},
{'name' : 'Event2', 'form_name' : 'test_form', 'event_date' : date(2015,1,1), 'capacity' : 1, 'close_date' : timezone.now() + timedelta(days=1),'description' : 'empty'},
{'name' : 'Event3', 'form_name' : 'test_form', 'event_date' : date(2015,1,1), 'capacity' : 1, 'close_date' : timezone.now() + timedelta(days=1),'description' : 'empty'}
]
